...
public class GetProfessionalByIdCommandHandlerTests
{
    private readonly Mock<IProfessionalRepository> _professionalRepositoryMock;
    private readonly GetProfessionalByIdCommandHandler _handler;

    public GetProfessionalByIdCommandHandlerTests()
    {
        _professionalRepositoryMock = new Mock<IProfessionalRepository>();
        _handler = new GetProfessionalByIdCommandHandler(_professionalRepositoryMock.Object);
    }
    ...
    public async Task Handle_WhenProfessionalExists_ShouldReturnProfessional()
    {
        ...
        result.Name.Should().Be(professional.Name);
    ...
    {
        // Arrange
        var command = new GetProfessionalByIdCommand { Id = Guid.Empty };

        // Act & Assert
        var exception = await Assert.ThrowsAsync<InvalidOperationException>(() => 
            _handler.Handle(command, CancellationToken.None));
        
        Assert.Contains("Professional ID cannot be empty", exception.InnerException?.Message);
    }
} 